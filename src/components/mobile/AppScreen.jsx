...
export default function AppScreen({ app, onClose, children }) {
  return (
    ...
    >
      {/* Title bar */}
      <div
        className="bg-[#EEF1EC] border-b border-[#DCE1DB]"
        style={{ paddingTop: "env(safe-area-inset-top, 0px)" }}
      >
        <div className="h-12 px-2 grid grid-cols-[4.5rem_1fr_4.5rem] items-center">
          <button
            onClick={onClose}
            className="flex items-center gap-0.5 pl-1 pr-2 py-2 text-xs text-[#5B8266] active:opacity-60"
          ...
          <span className="flex items-center justify-center gap-1.5 text-xs tracking-wide text-[#2E332F]">
      ...
      <div className="flex-1 min-h-0 overflow-auto p-4 text-sm text-[#3E453F]">{children}</div>
      ...
      <motion.div
        ...
        className="touch-none flex justify-center border-t border-[#DCE1DB] bg-white"
        style={{ paddingBottom: "env(safe-area-inset-bottom, 0px)" }}
      >
        <button onClick={onClose} aria-label="Go home" className="w-full py-3 flex justify-center">
          <span className="block w-28 h-1 rounded-full bg-[#2E332F]/30" />
        </button>
      </motion.div>
    </motion.div>
  );
